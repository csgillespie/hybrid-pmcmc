     ...
     1            istate, iopt, rwork, lrw, iwork, liw, jac, jt, params)
      external f, jac
      integer neq, itol, itask, istate, iopt, lrw, iwork, liw, jt
      double precision y, t, tout, rtol, atol, rwork, params
...
c  xerrwx, xsetun, and xsetf   handle the printing of all error
c           messages and warnings.  xerrwx is machine-dependent.
...
c
      data mord(1),mord(2)/12,5/, mxstp0/8500/, mxhnl0/10/
c-----------------------------------------------------------------------
c block a.
...
c initial call to f.  (lf0 points to yh(*,2).) -------------------------
      lf0 = lyh + nyh
      call f (neq, t, y, rwork(lf0), params)
      nfe = 1
...
c-----------------------------------------------------------------------
 ...
 280  if ((tn + h) .ne. tn) go to 290
      ...
      call xerrwx(50hlsoda--  warning..internal t (=r1) and h (=r2) are,
     1   50, 101, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(
     1  60h      such that in the machine, t + h = t on the next step  ,
     1   60, 101, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(50h      (h = step size). solver will continue anyway,
     1   50, 101, 1, 0, 0, 0, 2, tn, h)
      if (nhnil .lt. mxhnil) go to 290
      call xerrwx(50hlsoda--  above warning has been issued i1 times.  ,
     1   50, 102, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(50h      it will not be issued again for this problem,
...
c the maximum number of steps was taken before reaching tout. ----------
 500  call xerrwx(50hlsoda--  at current t (=r1), mxstep (=i1) steps   ,
     1   50, 201, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(50h      taken on this call before reaching tout     ,
     1   50, 201, 1, 1, mxstep, 0, 1, tn, 0.0d0)
      istate = -1
      go to 580
c ewt(i) .le. 0.0 for some i (not at start of problem). ----------------
 510  ewti = rwork(lewt+i-1)
      call xerrwx(50hlsoda--  at t (=r1), ewt(i1) has become r2 .le. 0.,
     1   50, 202, 1, 1, i, 0, 2, tn, ewti)
      istate = -6
      go to 580
c too much accuracy requested for machine precision. -------------------
 520  call xerrwx(50hlsoda--  at t (=r1), too much accuracy requested  ,
     1   50, 203, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(50h      for precision of machine..  see tolsf (=r2) ,
     1   50, 203, 1, 0, 0, 0, 2, tn, tolsf)
      rwork(14) = tolsf
      istate = -2
      go to 580
c kflag = -1.  error test failed repeatedly or with abs(h) = hmin. -----
 530  call xerrwx(50hlsoda--  at t(=r1) and step size h(=r2), the error,
     1   50, 204, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(50h      test failed repeatedly or with abs(h) = hmin,
     1   50, 204, 1, 0, 0, 0, 2, tn, h)
      istate = -4
      go to 560
c kflag = -2.  convergence failed repeatedly or with abs(h) = hmin. ----
 540  call xerrwx(50hlsoda--  at t (=r1) and step size h (=r2), the    ,
     1   50, 205, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(50h      corrector convergence failed repeatedly     ,
     1   50, 205, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      call xerrwx(30h      or with abs(h) = hmin   ,
     1   30, 205, 1, 0, 0, 0, 2, tn, h)
      istate = -5
      go to 560
c rwork length too small to proceed. -----------------------------------
 550  call xerrwx(50hlsoda--  at current t(=r1), rwork length too small,
...
c iwork length too small to proceed. -----------------------------------
 555  call xerrwx(50hlsoda--  at current t(=r1), iwork length too small,
...
c the run is halted.
c-----------------------------------------------------------------------
 601  call xerrwx(30hlsoda--  istate (=i1) illegal ,
     1   30, 1, 1, 1, istate, 0, 0, 0.0d0, 0.0d0)
      go to 700
 602  call xerrwx(30hlsoda--  itask (=i1) illegal  ,
     1   30, 2, 1, 1, itask, 0, 0, 0.0d0, 0.0d0)
      go to 700
 603  call xerrwx(50hlsoda--  istate .gt. 1 but lsoda not initialized  ,
     1   50, 3, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
      go to 700
 604  call xerrwx(30hlsoda--  neq (=i1) .lt. 1     ,
     1   30, 4, 1, 1, neq(1), 0, 0, 0.0d0, 0.0d0)
      go to 700
 605  call xerrwx(50hlsoda--  istate = 3 and neq increased (i1 to i2)  ,
     1   50, 5, 1, 2, n, neq(1), 0, 0.0d0, 0.0d0)
      go to 700
 606  call xerrwx(30hlsoda--  itol (=i1) illegal   ,
     1   30, 6, 1, 1, itol, 0, 0, 0.0d0, 0.0d0)
      go to 700
 607  call xerrwx(30hlsoda--  iopt (=i1) illegal   ,
     1   30, 7, 1, 1, iopt, 0, 0, 0.0d0, 0.0d0)
      go to 700
 608  call xerrwx(30hlsoda--  jt (=i1) illegal     ,
     1   30, 8, 1, 1, jt, 0, 0, 0.0d0, 0.0d0)
      go to 700
 609  call xerrwx(50hlsoda--  ml (=i1) illegal.. .lt.0 or .ge.neq (=i2),
     1   50, 9, 1, 2, ml, neq(1), 0, 0.0d0, 0.0d0)
      go to 700
 610  call xerrwx(50hlsoda--  mu (=i1) illegal.. .lt.0 or .ge.neq (=i2),
     1   50, 10, 1, 2, mu, neq(1), 0, 0.0d0, 0.0d0)
      go to 700
 611  call xerrwx(30hlsoda--  ixpr (=i1) illegal   ,
     1   30, 11, 1, 1, ixpr, 0, 0, 0.0d0, 0.0d0)
      go to 700
 612  call xerrwx(30hlsoda--  mxstep (=i1) .lt. 0  ,
     1   30, 12, 1, 1, mxstep, 0, 0, 0.0d0, 0.0d0)
      go to 700
 613  call xerrwx(30hlsoda--  mxhnil (=i1) .lt. 0  ,
     1   30, 13, 1, 1, mxhnil, 0, 0, 0.0d0, 0.0d0)
      go to 700
 614  call xerrwx(40hlsoda--  tout (=r1) behind t (=r2)      ,
     1   40, 14, 1, 0, 0, 0, 2, tout, t)
      call xerrwx(50h      integration direction is given by h0 (=r1)  ,
     1   50, 14, 1, 0, 0, 0, 1, h0, 0.0d0)
      go to 700
 615  call xerrwx(30hlsoda--  hmax (=r1) .lt. 0.0  ,
     1   30, 15, 1, 0, 0, 0, 1, hmax, 0.0d0)
      go to 700
 616  call xerrwx(30hlsoda--  hmin (=r1) .lt. 0.0  ,
 ...
 618  call xerrwx(
     ...
     1   60, 18, 1, 2, leniw, liw, 0, 0.0d0, 0.0d0)
      go to 700
 619  call xerrwx(40hlsoda--  rtol(i1) is r1 .lt. 0.0        ,
     1   40, 19, 1, 1, i, 0, 1, rtoli, 0.0d0)
      go to 700
 620  call xerrwx(40hlsoda--  atol(i1) is r1 .lt. 0.0        ,
     1   40, 20, 1, 1, i, 0, 1, atoli, 0.0d0)
      go to 700
 621  ewti = rwork(lewt+i-1)
      call xerrwx(40hlsoda--  ewt(i1) is r1 .le. 0.0         ,
 ...
 625  call xerrwx(
     ...
     1   60, 25, 1, 0, 0, 0, 2, tcrit, tout)
      go to 700
 626  call xerrwx(50hlsoda--  at start of problem, too much accuracy   ,
     ...
     1   60, 26, 1, 0, 0, 0, 1, tolsf, 0.0d0)
      rwork(14) = tolsf
      go to 700
 627  call xerrwx(50hlsoda--  trouble from intdy. itask = i1, tout = r1,
     1   50, 27, 1, 1, itask, 0, 1, tout, 0.0d0)
      go to 700
 628  call xerrwx(30hlsoda--  mxordn (=i1) .lt. 0  ,
     1   30, 28, 1, 1, mxordn, 0, 0, 0.0d0, 0.0d0)
      go to 700
 629  call xerrwx(30hlsoda--  mxords (=i1) .lt. 0  ,
...
c
 700  if (illin .eq. 5) go to 710
      ...
      istate = -3
      return
 710  call xerrwx(50hlsoda--  repeated occurrences of illegal input    ,
     1   50, 302, 1, 0, 0, 0, 0, 0.0d0, 0.0d0)
c
 800  call xerrwx(50hlsoda--  run aborted.. apparent infinite loop     ,
...
c blocks used by lsode and its variants.  the variables are
...
c-----------------------------------------------------------------------
      ...
      common /ls0001/ rownd, rowns(209), rcomm(9),
